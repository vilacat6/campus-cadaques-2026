...
export async function POST(request: Request) {
  try {
    ...
    if (error) {
      console.error("Error editant pagament:", error);

      return NextResponse.json(
        {
          error: "Error editant el pagament.",
    ...
    console.error("Error inesperat editant pagament:", error);

    return NextResponse.json(
      { error: "Error inesperat editant el pagament." },
      { status: 500 }
    );
  }
}